...
One honesty note on the pointer. The substrate repository is a post-submission extraction from this competition repository, existing or forthcoming on the maintainer's timeline; `docs/substrate-map.md` is the target of that extraction, not a claim that the forkable repository is live today. The pointer is honest either way, and this invitation does not present a not-yet-live link as though it were already serving.

## The honest claim, in one paragraph
