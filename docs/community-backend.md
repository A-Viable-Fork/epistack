...
Who a checker is, and the cost of a sock puppet. Contract today: named ids plus signed git commits, giving checker identity cryptographic teeth using machinery git already carries. Honest note: this yields attribution, not Sybil-resistance; the full answer is the standing economy, function 10. Tier: **assembly**.
...
- Zero-knowledge attestation gets a future-note, not an import: proof-of-verification for computational checks could someday close attestation fraud mechanically, and proofs over withheld data touch the withheld-record floor. Both are named as frontier-of-the-frontier, honestly.
