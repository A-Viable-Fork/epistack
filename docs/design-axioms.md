...
Depended on by: CLAUDE.md, docs/corpus-index.md, docs/exclusion-reservoir.md, docs/governing-trellis.md, docs/kernel-workflow-guide.md, docs/quickstart.md, docs/sorry-ledger.md, docs/status-ledger.md, docs/substrate-map.md
---
...
**T1-3. Perturbation is computed from the support graph, pinned to the authored cascade it reproduces (discharged, Prompt 21).** A flipped assumption removes the support it grounds and the collapse propagates along the support edges, the same removal-and-recompute the robustness analysis runs for the worst single removal, aimed at a chosen flip. The rule earned its way out of the exclusion reservoir by meeting the two conditions the hold named: the authored consequence cascade is retained as the verification fixture, and a rule audit exists and is asserted mechanically (`build/check-perturb.mjs`, propagation along support edges only, no collapse a lost support does not force). A wrong rule cannot mislead silently, because the computed cascade is checked against the authored ground truth and a mismatch stops and reports. `kernel/motions/perturb.js` computes the overlay non-destructively; honest beats impressive, and here the honest check is what let the computed engine ship.
...
- **Computed perturbation in v1.** ~~Excluded: silent-error risk (T1-3).~~ **Discharged (Prompt 21):** both conditions met, the authored cascade is retained as the verification fixture and the rule audit exists (`build/check-perturb.mjs`); perturbation is now computed from the support graph.
