...
Sharing your community is passing a **community card**: a small manifest whose durable identity is the governance-hash, the one named hash over the community's canonical parameter record, with the kernel name demoted to a human label over it exactly as a kind name is a label over a type-hash. The parameter record carries the pinned type hashes, the contribution target, the protocol identity, and the corpus content license, so all of them ride inside the governance-hash and travel visibly with every fork. The card also carries the snapshot hash, one or more fetch locations as transport hints, and a member-set commitment, nullable, the verification anchor for a membership-level identity presentation. Trust lives in the hash and not the host, so anyone can mirror the files and a community survives its host the way it survives its clients. Specified; the snapshot it points at is the static form of a kernel (build/emit-snapshot.mjs, built).
