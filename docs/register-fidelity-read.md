...
Refreshed (attenuation rewrite, July 2026): the `opening` and `what-knowledge-is` rows were re-read against the current `precise_version` after the attenuation rewrite advanced those precise sections to the attenuation frame; the other rows grade correspondences the rewrite left in substance and stand as written.

## The verdicts

| Section | Verdict | What the read confirmed |
|---|---|---|
| opening | Faithful | The enacted concession is promoted to a stated commitment ("the argument depends on this strength being real"), which avoids the glib-about-synthesis failure; the synthesis limit is located structurally, one viewpoint's reach, with the brief's consequences following from it rather than stated as a list of failures, and the accessible carries that limit-first-then-consequences order. Both moves check out against the rewritten precise opening. |
| what-knowledge-is | Faithful | The constitutive definition "the invariant left as the model is attenuated" is translated to a plainer figure of the same kind ("what is left of a claim as trust in whoever produced it is turned down"), a dial driven down rather than a switch, with the delta keeping "subtraction" by name as the zero-limit only self-securing types reach; the earned analogy "one measurement is testimony; many are knowledge" is kept verbatim; the synthesis-placement paragraph carries the whole placement (one viewpoint, the hold-and-reach cycle, the self-securing-proof exception), not a dismissal. |
