...
| Compounds across people and domains | The composition layer, the shared vocabulary, and the tamper-evident patch history, which open the trust market (a participant chooses which grounding to stand under and can fork any kernel at the untyped type) alongside the interface market (a participant chooses which surface to read and write through, over the contract) | `kernel/composition/`, `kernel/store/patch-ledger.js` | Built at minimum scale; both markets exposed and unpopulated at large scale, which is adoption |
...
| Teaching uplift: bringing a reader up to speed | Modest by design, which is what the criterion's benchmark of off-the-shelf research and coding assistants measures: the thin periphery surfaces claim structure, what is load-bearing, where an argument turns, what is unmeasured, rather than trying to explain a topic faster than a strong synthesis does. No longer only argued: the register view is a built surface that shows it, an accessible reading of the judges document grounded on the same nodes as the precise reading | `periphery/` (the thin demonstration surface), the register view (`docs/register-view.md`, the accessible register grounded under both registers), the guided path in `submission.html` (the three cases walked) | Present, real, not the contribution; now shown, not only asserted |
| Claim uplift: the claim itself made checkable, composable, and able to survive the removal of trust | Two components. Present: mechanical grounding makes a claim's standing checkable, the exclusion record makes a refutation stick, provenance makes an origin traceable, the crux locates a disagreement's frontier. Enabled: separating trust from view lets the interface market, surfaces composing over shared checkable trust, form independently of the trust market, so claim uplift compounds across many peripheries, enabled and not guaranteed, the credit sought being for the enablement the compounding and scaling criteria ask the evaluator to weigh; the register view is the first realized view in that market, two teachers over one trust base, so the enabled component is now shown on one instance rather than only argued | `kernel/grounding/`, `kernel/analysis/` (present, on the three cases), the register view (`docs/register-view.md`, the first realized second view), `docs/epistemic_uplift.md` and `docs/what-stands-without-trust.md` (the axes and the scoping note), the external contract's feasibility (the enablement) | Present component built; enabled component argued and shown on one instance |
