...
| Stays usable against a changing user base and expanding tooling | The untyped type plus retail and wholesale forks plus standards-from-use: domains keep their own schemas, meet at the untyped type that grounds nothing, and a shared vocabulary grows from accumulated forks rather than by decree | `kernel/composition/` (crossings, forks), `docs/what-stands-without-trust.md` (federation appendix) | Top-down built; bottom-up federation specified |
