...
| Stays usable against a changing user base and expanding tooling | The untyped type plus retail and wholesale forks plus standards-from-use: domains keep their own schemas, meet at the untyped type that grounds nothing, and a shared vocabulary grows from accumulated forks rather than by decree | `kernel/composition/` (crossings, forks), `docs/what-stands-without-trust.md` (federation appendix), `docs/adversarial_walkthrough.md` (scenario five: the border refusing to launder standing) | Top-down built; bottom-up federation specified |
| Epistemic uplift, and generalizing beyond the three cases | The interfaces a checkable structure unlocks that synthesis cannot support, and each of the competition's named strong examples mapped to a kernel capability | `docs/epistemic_uplift.md` (the case argued on the competition's own examples), the guided path in `submission.html` (the three cases walked) | Argued; the interfaces the periphery ships are in `periphery/` |
...
The trust boundary is what lets a stronger model improve the periphery without touching what the core guarantees, checked as a property of the import graph by `build/check-map.mjs`. The composition layer is what lets a question span domains without merging them into one floor: nutrition, environment, and economics ground to their own floors and a spanning question composes them, in `kernel/composition/`. Maintainability is the same machinery read forward in time: because the untyped type is not a floor and nothing grounds through it, a new domain or a foreign schema meets the tree at the untyped type and earns standing only by an owned fork, retail per claim or wholesale per category, so the format absorbs a changing user base and new tooling without a central schema authority legislating the additions, and the shared vocabulary is what accumulated forks have proven in use.
