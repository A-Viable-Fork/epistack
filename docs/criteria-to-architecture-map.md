...
| Find the decisive disagreement | The contradiction record with its divergence-point set, and the crux computation over it | `kernel/schema/records.mjs` (record) | Record built; the crux computation is specified, see the status ledger |
