...
Depends on: docs/kernel-taxonomy.md, docs/knowledge-system-what.md, docs/status-ledger.md, docs/what-stands-without-trust.md
...
The two uplift rows are orthogonal, and the split matters for how the criterion is read. A better synthesis raises teaching uplift while leaving the claim beneath it unchecked, because in a synthesis the claim was never a separable object; this submission raises claim uplift and is modest on teaching, so a teaching-axis benchmark cannot register its contribution, which is the gap in the instrument that `docs/reading-the-brief.md` names and the scoping note in `docs/what-stands-without-trust.md` develops. The deepest form of claim uplift is the production of knower-independent knowledge, a claim that stands when trust in whoever asserted it is subtracted, which is the definition of knowledge the whole submission runs on, so the uplift axis and the definition line up: to raise claim uplift to its limit is to make the claim hold without its knower.
