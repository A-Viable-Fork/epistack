...
**The gate.** Admission is a single comparison. A proposal is accepted only if the grounding it declares is at or below the effective grounding its supports actually provide. You cannot bank grounding you cannot cover: declare measurement while resting on a judgment and the effective value returns forum, the declared value exceeds it, and the gate rejects the proposal or forces it to re-declare at the grounding it can support. This is also why the bottom of the lattice can be left wide open. A raw, untyped claim has effective grounding at the bottom, and nothing grounded can take a necessary support edge from it without collapsing to the bottom itself, so unfiltered material is safe to admit at the cheapest tier. The typing is the claimant's and not the gate's: an author types a claim and stakes their own standing on it, and the gate checks the typing it is handed rather than manufacturing typing for raw input. That author is any named author, a person, a model, or an agent pipeline alike, and the gate never inspects which, so the identical admission falls on all three.
...
The **ingestion pipeline** is the producer that reads messy real-world sources and emits attributed candidate claims with proposed types and links, then submits them through the propose interface. Extraction and first-pass typing happen here; the gate is what makes them safe to admit. Ingestion lives in the periphery for the same reason semantic judgment does: a kernel that ingested at its center would be a trusted synthesizer, the single high-leverage target an adversary games once to launder everything downstream, the critique's own object installed at the center with a stamp. In the periphery ingestion is producer work whose output faces the gate like any other producer's, checkable and producer-agnostic, so a compromised ingester compromises only its own submissions. The **authoring and curation tools** let a person, AI-assisted, add claims, propose groundings, discharge open questions, and record refutations. The **navigator** is the family of reading surfaces, a static rendering that simply shows the map, a light client that walks it, and a rich client that drills in and runs what-if explorations, all read-only.
