...
**[1.3] The forum region.** The contested region with its tiers, raw, attributed, structured, and its two subtypes: an open question whose ceiling is a floor (a real debt) and an unsettleable judgment whose ceiling is structured forum (settled once structured). In progress: the v3 order carries the open-line grades and a `forum` kind whose ceiling is corroborated, and the forum path is now exercised by a REAL case, the migrated COVID conclusion terminates at a priced-prior forum (`build/check-migrate.mjs`), not just a fixture. The finer tier progression (raw, attributed, structured) as an authored ladder on real content remains.
