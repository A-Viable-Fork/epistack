...
**Stage 0 open obligations** (in `docs/sorry-ledger.md`): four deferred verifications (the two LHC kinematic checks, the two case instances). The LHC accretion branch (`lhc.branch3`) is now authored to the floor: N3.1 (the regime-dependent accretion rate) and N3.2 (the destroy-time and the survival contradiction) are drawn to cited Giddings-Mangano (arXiv:0806.3381) primitive leaves, read from the source rather than reconstructed from the Bondi reference form, so the rate is piecewise and captures the three departures from naive four-dimensional Bondi (regime crossover, the D-dimensional Bondi radius, and the nuclear-matter geometric collapse). The LHC cascade is now authored across all three branches (production, stopping, accretion); discharging the four remaining verifications closes the formal floor on all three cases. The generalization of the derivation segment to every measurement claim is a characterized leap, and the two-stage factoring of the contested statistic is a second; both close with further instances.
