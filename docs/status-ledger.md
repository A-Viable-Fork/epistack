...
**Stage 0 open obligations** (in `docs/sorry-ledger.md`): two deferred verifications, both population-case instances (`covid.instance`, `eggs.instance`). The LHC case is now fully grounded: all three branches are authored to the floor (production, stopping, accretion, the last read from Giddings-Mangano arXiv:0806.3381 rather than reconstructed from the Bondi reference form), and the two Branch-2 verifications are discharged against the same source. `lhc.N2.1` (the production kinematic factor `gamma_BH ~ sqrt(s)/(2 m_N)`) is confirmed at G-M eqs (5.3)-(5.5) and fig. 7, with the LHC clause corrected to non-relativistic-but-mostly-above-escape-velocity; `lhc.N2.2` (the charged/neutral stopping split and its body assignment, charged: Earth and the Sun; neutral: white dwarfs and neutron stars) is confirmed at G-M Sections 2.2 and 5.2. Discharging the two remaining population-case instances closes the formal floor on all three cases. The generalization of the derivation segment to every measurement claim is a characterized leap, and the two-stage factoring of the contested statistic is a second; both close with further instances.
