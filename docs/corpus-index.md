...
| `corpora/lineage/tcpip-contracts.js` | the internet lineage's per-era trust contracts as structured data: one row per trust decision per act (decision, era policy, enforcement holder, documented failure, grounding net.* refs), feeding the coming counterexample-kernel exhibit. Pure data, wired into no build yet |
