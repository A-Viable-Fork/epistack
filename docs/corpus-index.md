...
| `kernel/schema/glossary.mjs` | the self-describing vocabulary (DESCRIBE-1): a description and whenToUse for every grade in the confidence order, a description and ceiling for every shared-root claim kind, a description for every link kind, and the declared-vs-earned concept, keyed to the real schema by identity |
...
| `corpora/compute/stats-pack.js` | the statistics pack (COMPUTE-1): the plural, forkable statistics transformations, naive-multiply and dependence-aware, that disagree on purpose over the covid corpus's own suboptimal-PRRAR factors; corpus content, not shared root, since there is no canonical Bayesian aggregation |
...
| `corpora/lineage/tables.js` | the lineage case (fourth and fifth cases) reference tables: a source row per cited report/study/document/RFC, and three kinds (measurement/checked, forum/corroborated, declaration/constitutive). The deep-research reports are canonical for the claims, never for the grade |
| `corpora/lineage/lineage.js` | the lineage case authored from deep-research reports: the kernel's mechanisms already run by hand in common law, software, science, Wikipedia, journalism, and accounting, the novelty conjecture that no known system composes all five axes, and the fifth case, the internet's own trust lineage (net.*) across three acts with its recentralization-versus-federation contest held open. Declared conservatively; the gate prices it and the demotions are the finding |
| `corpora/lineage/tcpip-contracts.js` | the internet lineage's per-era trust contracts as structured data: one row per trust decision per act (decision, era policy, enforcement holder, documented failure, grounding net.* refs), read by the counterexample-kernel exhibit to map each computed violation to a documented failure. Pure data |
...
| `corpora/self/stage-0.md` | the self-kernel frame: what the reflexive kernel governs (the protocol's claims about its own properties, definitions, and theses, in three registers) and what it does not (the case domains, the math and vocabulary kernels it cites, the engine code the checks test), store_id S-self |
| `corpora/self/tables.js` | the self-kernel reference tables: the three adopted shared kinds (measurement for invariants, declaration for definitions and entrance stipulations, forum for theses) with their pinned hashes, and the sources its claims cite (the oracle suite, the math kernel, the vocabulary kernel, and the theses) |
| `corpora/self/self-data.js` | the self-kernel store (S-self): epistack's grounded claims about itself, eleven structural invariants (measurement) each citing a math theorem for why and a check for that, one floored as the history gap, four constitutive definitions adopting vocabulary terms, three evaluative theses at the forum floor, and the entrance-surfaced listing (title, tagline, status, link) for the org-root door |
...
| `build/check-algebra.mjs` | the kernel-algebra oracle: closure, the governing law on fuzzed pairs (compose floors every crossed claim, project never lifts, diff asserts nothing unread), compose associativity up to hash and project distributing over compose with the grade-band boundary characterized, diff correctness, and the eggs composite-versus-join exhibit |
...
| `build/self-build.mjs` | the self-kernel builder: builds S-self through the real gate from corpora/self, so the oracle grounds the same structure the gate computes; adds no rule of its own |
| `build/check-self.mjs` | the self-kernel oracle (the reflexive exhibit): builds S-self, resolves every citation (each cited math theorem exists and grounds, each cited check exists, each constitutive claim adopts a real vocabulary term), and confirms the gate's grade matches the register; fails on a dangling citation, an overclaim, or a definitional constitutive claim with no vocabulary entry, and confirms every floored invariant carries a sorry-ledger entry |
| `build/check-boundary.mjs` | the boundary linter: holds the contract register (required tier) and the parameters register (free tier) disjoint, checks no marked claim asserts the wrong modality (a free parameter as an invariant, a contract as a choice), that every composability invariant and ecosystem-guide invariant resolves to a contract entry, and that every marker and cross-kernel reference resolves |
| `build/check-conformance.mjs` | the conformance checker: runs the contract register against an arbitrary foreign kernel (in the algebra shape) and emits a per-invariant violation report (conforms / violates / unverifiable, with evidence) rather than a pass/fail; register-driven, so its invariant set equals the parsed contract register; five invariants have a mechanical predicate over the advertised-versus-recomputed grade gap, the rest are named unverifiable-mechanically; verifies a conforming and a non-conforming fixture and a baked-in deliberate-break flip |
| `build/tcpip-eras.mjs` | the three internet-era kernels for the counterexample exhibit: act1Kernel/act2Kernel/act3Kernel built with foreignKernel, each faithfully encoding the era's trust decisions (acceptances, not content) sourced from the grounded net.* claims, carrying a mapping from each acceptance to the net.* refs it encodes |
| `build/check-tcpip-counterexample.mjs` | the TCP/IP counterexample exhibit oracle: runs the conformance checker over the three era kernels, verifies the eras hold the composition invariants and violate the control invariants, that Act 2's profile contains Act 1's plus the untyped-floor exploit, that Act 3's CT and signed-ROA elements recompute while its authority grants do not, maps every violation to a documented failure in tcpip-contracts.js, and confirms the exhibit doc's verdict tables match the computation (anti-drift) |
...
| `build/check-comment.mjs` | the comment kind oracle: confirms grounding inertness (a comment thread, added or withdrawn, moves no existing grade or certificate), the comment-bomb rejected by name, replies-to thread shape resolving like any other link, and the ceiling held under fuzzed support |
...
| `build/check-compute.mjs` | the transformation-store oracle (COMPUTE-1): the entry shape, the canonical packs registered by reference, the covid acceptance test (naive-multiply and dependence-aware disagreeing over the debate's own RRAR-site and proline factors), and the no-standing rule confirmed mechanically on real covid-derived data |
| `build/check-glossary.mjs` | the self-describing-vocabulary oracle (DESCRIBE-1): completeness against the real schema both directions, the declared-vs-earned concept load-bearing and naming both sides, the client contract's glossary() serving the kernel's export unchanged, and a real covid mode-split pair printed as the worked example |
...
| `docs/for-the-institutional-adopter.md` | the institutional entrance: what a public typed claim graph offers an organization that must make defensible decisions in contested domains, accountability as attributability plus recomputability plus isolability, adoption as a governance-hash parameter choice, exit by fork, and the three cases walked as an adopter would; an argument, specified where it cites the coordination layer |
| `docs/knowledge-kernels-and-crypto.md` | the DOs and DON'Ts of connecting knowledge kernels to on-chain finance, branch-local to lineage-defi-branch: the plural-bridge theorem, what a market layer may reference (grades) and never move, the corpus content license as a market-facing consequence, and the for-profit verifier and transcribed-work royalty arguments; specified throughout, nothing in it built |
| `docs/submission-overview.md` | the submission's first page: the approachable, judges-facing overview of EpiStack and The Knowledge Game, the six critiques and their counterexamples, the protocol in a paragraph, the community layer, the app, and the two reader paths; deliberately front-matter-free (outward-facing, usable verbatim as the competition-form overview), excluded from the argument graph like the generated repo map. Now framed downstream of the minimum constitution: the opening and each critique lead read the six pairs as regime-evidence, not attacks |
| `docs/the-minimum-constitution.md` | the submission's first read (the frame the reading paths inherit): what EpiStack is, why trust-in-producer was a well-calibrated basis whose regime shifted, why rebasing rather than reinforcement follows (the Ashby variety argument), and why the fixed/free boundary falls at structural versus semantic attenuation, read off the contract and parameters registers. Affirmative, regime-conditional; the README's judge entry and the prose home of the self-kernel's minimum-constitution thesis (`self.thesis.minimum-constitution`) |
