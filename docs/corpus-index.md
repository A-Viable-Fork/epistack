...
| `periphery/navigate/shell/modules/manager.js` | the kernel manager, slice two: renders the federation as multi-author kernels with their tiers and crossings, driving through the management contract (listKernels, readKernel, readCrossings) and the claim contract (live grade), and performs all three writes (adopt, fork, cross) through the contract with real receipts; vendors no answers, holds no grounding logic |
