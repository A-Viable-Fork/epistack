...
| `corpora/math/math-data.js` | the math kernel store: the stage-zero axioms of the confidence order entered as declaration-claims at the constitutive floor (the grade set, the partial order, the empirical/constitutive mode incomparability), the given basis the later lattice-law and recurrence claims ground against |
...
| `build/check-math.mjs` | the math kernel oracle (sixth exhibit, in progress): verifies the generated kernel is coherent (adopted hashes match the shared subtree, tables build, the gate accepts) and, from stage zero, that the confidence-order axioms are present as declarations earning the constitutive floor |
