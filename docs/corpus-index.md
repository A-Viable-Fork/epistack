...
| `build/vendor-federation.mjs` | freezes the bottom-up federation into vendor/federation/federation-view.json for the kernel-manager surface: the four members (author, provenance, local-versus-shared tiers, a sample claim), the crossings marked native or untyped, and the fork-and-adopt probe with the real before and after status from the adoption layer |
...
| `periphery/navigate/shell/modules/manager.js` | the kernel manager, slice one (the probe): renders the federation as multi-author kernels with their tiers and crossings, reads a member's live grade through the contract, and performs one real fork-and-adopt (pinning a type-hash so an untyped crossing composes native); holds no grounding logic |
| `build/local-management-provider.mjs` | the local provider behind the management contract: implements listKernels, readKernel, readCrossings, adopt, fork, and cross over the real federation and adoption layer, holding session pin-state; lives in build/ because it wraps build-layer machinery api/ may not import |
