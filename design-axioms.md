...
**T0-2. Three layers, never crossed.** `data/` is pure values, JSON and plain objects, no behavior. `engine/` is pure logic with no DOM reference, no `window`, no `document`. `view/` reads the engine and renders, and owns no data of its own. The dependency arrows point one way: view depends on engine depends on data, never the reverse. This is what makes the engine runnable headless for the compose gate and the linter, and what makes the renderer swappable. A DOM call in `engine/` or a data literal in `view/` is a Tier 0 violation.
...
4. No DOM reference in `engine/`; no `window` or `document`. No data literal in `view/` that should come from the engine. (T0-2)
