  ...
  var SECTIONS = [
    {
      ...
      precise_version: "155847b5767e001d",
      ...
      accessible:
        ...
        "Start where the competition starts, with synthesis, what the best deep-research tools produce. The brief asks for work that compounds across teams, survives adversarial reading, and grows with more contributors. A synthesis is one viewpoint's reach, so on its own it gives none of these: the next person has to trust it or redo it, a determined challenge meets the synthesizer's authority instead of a shared floor, and two of them over one question do not combine. What the brief asks for rules out the thing almost everyone will build. The critique is what is left once those demands are taken seriously: knowledge has to be what stands when trust in the author is removed, because everything the brief wants is a property of claims that survive without their author. Three cases show what one viewpoint's reading cannot surface on its own, a reified dependency in physics, a hidden prior in epidemiology, a swappable frame in nutrition.\n\n" +
        "That forces the protocol, and the protocol is a climb. If knowledge is what stands once the author is removed, something has to do the removing mechanically, and building it is where the critique leads. First the type: a knowledge kernel, a fixed core that grounds claims and lints them in CI, with two markets over it, kernels to trust and surfaces to read and write through. Then one instance, this submission's top-down meta kernel, one schema over the three domains, built and checked. Then the generator, which builds a working kernel from a config, proven by regenerating the cases. Then the proof the generator makes real kernels: the same four cases federated bottom-up as standalone members, with a check that standing crosses only through an owned fork, so no member's authority reaches another. Each link proves the one before it.\n\n" +
        "The climb is built through the generator and its federation, and checks verify each step. Above it sits the specified frontier, the coordination fabric a large federation needs to govern its writes, designed and marked as such. The critique is complete, its three cases worked and checked. The same three cases run the whole way up: the critique's counterexamples, then the instance's demonstration, then the generator's output, then the federation's members. A fourth exhibit, the lineage, shows these mechanisms already run by hand in mature institutions, and that composing them is new. If you read one thing, read the critique and its three cases. If you then run one thing, run the oracles, each of which states the invariant it checked.\n\n" +
        ...
        "A synthesis is the most one reasoner can do, and that is real. Its limit is not weakness, it is structural: a synthesis is one viewpoint's reach, and from inside a single view you cannot separate what is the world from what is the vantage, because the synthesizer's trust runs through it and cannot be removed from within. The rest follows. The next person has to trust it or redo it, because its judgments, this claim is solid, that one is just rhetoric, this is the crux, are the synthesizer's own conclusions, not a structure you can check and extend. Two syntheses of the same question do not combine; merging them puts a person back in the loop. Against someone who disputes a claim it offers the synthesizer's authority, not a floor the disputant also stands on. Cheap generation sharpens this rather than softening it: the better and cheaper the synthesis, the clearer that even the best is still one viewpoint, and the trust it asks for is the whole of what it offers. So a synthesis is read, and then it ends, not because it failed to travel but because being one viewpoint's reach is what it is.\n\n" +
        "Why the best synthesis stops short comes down to what \"knowledge\" has to mean once more than one reasoner is involved.",
      delta:
        "The precise register opens with the how-to-read frame that names the submission as a critique with counterexamples and the protocol it forces; this keeps the frame and its honesty line (the critique complete, the protocol a built climb through the generator and its federation with the coordination fabric the specified frontier), preserves the brief's quoted shape-labels, and collapses only the frame's repetition. It enacts the concession through cadence and first-person address; this states the commitment plainly and loses the enacted sincerity, and it collapses the capability ramp to one list. The precise register locates the synthesis limit structurally, a synthesis is one viewpoint's reach, then lets the brief's consequences follow from it; this keeps that order, the limit first and the consequences second, in plainer words, and states the sharpening, that a better synthesis makes the point clearer, without the precise register's compression.",
    ...
    },
    {
      ...
      precise_version: "a405b0fbc37a79ec",
      ...
      accessible:
        ...
        "The same subtraction places synthesis. Knowledge is what stands across viewpoints, the invariant left when each synthesizer's trust is subtracted, made of syntheses and equal to none of them. It is also what a synthesis is built from, the settled ground a new view starts on, so the two are a cycle: knowledge is made of syntheses and is the ground the next one starts from, and no single synthesis is itself knowledge. A synthesis is one reasoner reading the record and reaching a view, the individual act any body of knowledge is built from, and it stands to knowledge as one measurement stands to the measured constant. So the complaint that a synthesis ends is not that it is weak but that it is one viewpoint, and one viewpoint is not knowledge until the synthesizer is subtracted, which a lone synthesis cannot do to itself. One case is the exception: where a claim carries its own check, a proof, the synthesizer is subtracted from inside a single synthesis, because anyone can check the steps, so one reasoner establishes knowledge with no agreement across others needed, which is why formal domains are the exception. Everywhere else a synthesis cannot secure itself, so knowledge is the cross-viewpoint invariant. The kernel does not replace the reading; it composes many, so a synthesis enters as claims to be grounded and a view to be chosen among, and the individual reading becomes part of what stands without its author.\n\n" +
      ...
      verify: [
        ...
        "synthesis-placement paragraph (one viewpoint, the foothold cycle, the self-securing-proof exception); check it carries the whole placement, honoring synthesis, not a dismissal"
      ]
    },
    {
    ...
    },
    {
      ...
      precise_version: "cc16745bf3050fd4",
    ...
    },
    {
      ...
      precise_version: "35c737074c4180e4",
      register_link: "paper-5",
      accessible:
        "The bottom-up meta kernel is built, the four cases federated as standalone members and verified, and it is the arrangement for a different regime, many authors who will never share a schema. Independent kernels, each owning its own types, join at the untyped type; composites pick members per question; standing crosses only by owned forks; and retail forks pile up into wholesale ones, so a federation grows a shared vocabulary out of use, the way a path becomes a road because people kept walking it. check-bottomup runs the four cases as standalone members and confirms adversarially that each owns its schema, that standing crosses only through an owned fork, and that same-hash crossings compose native. What a community sets for itself and what every kernel must hold to stay composable is drawn by the parameters register: a free tier a community configures, time locks, standing rules, agent policy, type system, and a thin required tier no kernel may break without leaving the protocol. Diversity in the free tier, unity in the required tier, is the same taxonomy read as governance. The submission built the right thing for one author over three domains, and it built and checked the federation for many, with the coordination fabric that governs writes at scale the specified frontier above it.",
      delta: null,
    ...
    },
    {
      ...
      precise_version: "e2d403d3ac22ed89",
      ...
      accessible:
        ...
        "The members are domain kernels that already exist and were built without coordination, a physics kernel with proof and measurement floors, an epidemiology kernel whose floors are estimates with stated methods, a policy kernel whose floors are declarations. Each is standalone; nothing in the federation can change a member's types from outside, and that independence is the point, because it lets a community adopt the protocol without first agreeing with anyone.\n\n" +
