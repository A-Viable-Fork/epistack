...
const terms = [
  ...
  t("vt.community-card", "community card", "community card: the manifest naming a community by its kernel identity and snapshot hash, with its pinned type hashes and contribution target, and one or more fetch locations as transport hints, so trust lives in the hash and not the host.", "docs/ecosystem-guide.md", "s", { departure: "small" }),
  t("vt.identity-claim", "identity claim", "identity claim: the claim that a key is a persistent agent with attributes, graded by its attestation supports through the same recurrence that grades everything else, and grading the identity claim rather than the person.", "docs/ecosystem-guide.md", "s", { departure: "small" }),
  t("vt.registry", "registry", "registry: a kernel whose domain is artifacts, its types interface contracts hashed beside their conformance oracles and its grades computed from conformance rather than quality, plural and forkable, of which a community directory is one instance.", "docs/ecosystem-guide.md", "s", { departure: "small" }),
  ...
  t("vt.contract-register", "contract register", "contract register: the enumerated required tier, the composition invariants a kernel must hold to stay composable, the checkable complement of the parameters register free tier.", "docs/contract-register.md", "c"),
  t("vt.kernel-algebra", "kernel algebra", "kernel algebra: the closed set of operations over kernels as objects, each taking kernels and returning a kernel so they chain, governed by one law, that no operation assigns a grade above what recomputation in the result confirms.", "docs/composition-spec.md", "c", { departure: "small" }),
  t("vt.project", "project", "project: the algebra operation restricting a kernel to the subgraph its claims satisfying a predicate, recomputing grades over the restriction so a projected grade can only fall, never rise.", "docs/composition-spec.md", "c", { departure: "small" }),
  t("vt.diff", "diff", "diff: the algebra operation reading the structured difference of two kernels over the shared claim skeleton, returned as a diff-kernel that asserts nothing it did not read.", "docs/composition-spec.md", "c", { departure: "small" }),
  t("vt.kernel-dialect", "kernel dialect", "kernel dialect: a kernel type with a numeric ceiling and its own gate, adopted by the communities that pin it, federating with ordinal kernels only through the crossing at the untyped floor because they share no grade semantics to transfer.", "docs/composition-spec.md", "s", { departure: "medium" }),
  ...
  t("vt.governance-hash", "governance-hash", "governance-hash: the content-addressed hash over a community's full parameterization, certifying the contracts a community runs so a claim's standing composes across communities under a frozen, inspectable meaning.", "docs/coordination-layer-spec.md", "s"),
